...
import com.kemar.olam.sales_and_inspection.ground_sales.adapter.GroundApprvalLogDeailsAdapter
...
class GroundApprovalLogsDeatilsFragment : Fragment() ,View.OnClickListener, DialogFragment.GetDialogListener {
    ...
    lateinit var adapter: GroundApprvalLogDeailsAdapter
    ...
    var historyModel: LogsUserHistoryRes.BordereauGroundList = LogsUserHistoryRes.BordereauGroundList()
    ...
    var firstImagePath = ""
    var secodnImagepath = ""
    var strInspection_number: String? = ""
    ...
    override fun onCreateView(
        ...
        mView = inflater.inflate(R.layout.fragment_ground_approval_logs_deatils, container, false)
        initViews()
       /* CreateProfile()
        setDecoderValues()*/
        return mView
    ...
    fun initViews() {
        ...
        if (commigFrom.equals( Constants.header, ignoreCase = true)) {
            ...
            if (headerDataModel != null) {
                ...
                strInspection_number = headerModel.inspectionNO?.toString()
            ...
            if (headerDataModel != null) {
                ...
                strInspection_number = historyModel.inspectionNumber?.toString()
        ...
        mView.swipeLogListing.setOnRefreshListener {
            ...
            inspectionID?.toString()?.let { getGroundsInspectionIdsAllLogs(it) }
        }
    ...
    fun setupRecyclerViewNAdapter() {
        ...
        adapter = GroundApprvalLogDeailsAdapter(mView.context, groundLogsLiting, commigFrom)
    ...
    fun generateConfirmForApproveRequest(): AddBoereuLogListingReq {
        ...
        request.setBordereauLogList(groundLogsLiting)
    ...
    fun generateConfirmForBillingRequest(
        ...
        try {
            ...
            request.setInspectionDate(mUtils.getCurrentDate())
            request.setUserID(SharedPref.getUserId(Constants.user_id))
            request.setInspectionNumber(strInspection_number)
            ...
            request.setSelectedLogs(groundLogsLiting.size)
            request.setBordereauLogList(groundLogsLiting)
            var json = Gson().toJson(request)
            var test = json
  ...
  */
   ...
   /* private fun saveBitmapIntoStorage(bmp: Bitmap): File? {
        ...
        if (mUtils?.checkInternetConnection(mView.context) == true) {
            try {
                ...
                val request = generateConfirmForBillingRequest(
                    represetativeSign,
                    cuustomerSign
                )//generateJsonStringrequestForBasee64(represetativeSign,cuustomerSign)
    ...
    fun getGroundsInspectionIdsAllLogsRequest(inspectionID: String): CommonRequest {
        val request : CommonRequest =   CommonRequest()
        request.setInspectionId(inspectionID)
        val json =  Gson().toJson(request)
        var test  = json
    ...
    private fun getGroundsInspectionIdsAllLogs(inspectionID:String) {
        if (mUtils.checkInternetConnection(mView.context)) {
            try {
                mUtils.showProgressDialog(mView.context)
                val apiInterface: ApiEndPoints = ApiClient.client.create(ApiEndPoints::class.java)
val request   =  getGroundsInspectionIdsAllLogsRequest(inspectionID)
                val call_api: Call<GetBodereuLogByIdRes> =
                    apiInterface.getGroundBodereuLogsByID(request)
                call_api.enqueue(object :
