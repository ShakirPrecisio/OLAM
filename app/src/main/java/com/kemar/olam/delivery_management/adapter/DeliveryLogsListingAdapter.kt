...
class DeliveryLogsListingAdapter  (val context : Context, val dataList: ArrayList<BodereuLogListing>, val commingFrom:String) : RecyclerView.Adapter<DeliveryLogsListingAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.txtCustomerName.text=dataList[position].getcustomerPurchasedFromForest()
        ...
        holder?.ivRemove?.setOnClickListener{
            dataList[position]?.let { it1 -> onDeleteClick?.invoke(it1,position) }
        }
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, position: Int): ViewHolder {
        val v = LayoutInflater.from(viewGroup.context).inflate(R.layout.row_view_delivery_logs_liisting, viewGroup, false)
        return ViewHolder(v);
    }
