...
public class PDFTableView extends PDFView implements Serializable {
    ...
    public PDFTableView addRow(@NonNull Context context,@NonNull PDFTableRowView rowView,@NonNull Boolean isLineNeeded) {
        if (this.rowWidthPercent.length > 0) {
            rowView.setColumnWidth(this.rowWidthPercent);
        }
        PDFVerticalView verticalView = new PDFVerticalView(context);
        verticalView.addView(rowView);
        if(isLineNeeded){
            verticalView.addView(new PDFLineSeparatorView(context).setBackgroundColor(Color.BLACK));
        }
        super.addView(verticalView);
    ...
    public static class PDFTableRowView extends PDFHorizontalView implements Serializable {
        ...
        public PDFTableRowView addToHeaderRow(@NonNull PDFTextView TextViewToAdd) {
        ...
        public PDFTableRowView addToCustomRow(@NonNull PDFTextView TextViewToAdd) {
            LinearLayout.LayoutParams txtLayoutParam1 = new LinearLayout.LayoutParams(
                    0,
                    LinearLayout.LayoutParams.MATCH_PARENT, 1);

            txtLayoutParam1.setMargins(10, 0, 0, 0);
            TextViewToAdd.setLayout(txtLayoutParam1);
            super.addView(TextViewToAdd);
